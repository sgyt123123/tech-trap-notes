...
import { BookOpen, Hash, Lightbulb, MousePointerClick } from 'lucide-react';
...
const InfoPanel: React.FC<InfoPanelProps> = ({ selectedNode, onNavigate }) => {
  ...
  if (!selectedNode) {
    return (
      <motion.div
        className="h-full flex flex-col items-center justify-center text-slate-300 p-12 text-center bg-slate-900/90 backdrop-blur-md"
      ...
      >
        <div className="relative mb-8 group cursor-default">
          ...
          <div className="bg-slate-900 p-8 rounded-3xl border border-slate-600/70 relative z-10 shadow-2xl group-hover:scale-105 transition-transform duration-500">
            <BookOpen size={48} className="text-cyan-500" />
          </div>
        </div>
        <h3 className="text-3xl font-serif font-bold text-white mb-4 tracking-wide">探索机制</h3>
        <p className="text-slate-200 leading-relaxed max-w-xs">
          点击左侧图谱中的节点
          <br />
          解构<span className="text-cyan-400 font-bold mx-1">技术陷阱</span>的深层逻辑
        </p>
        <div className="mt-8 flex items-center gap-2 text-xs text-slate-300 animate-pulse">
  ...
  return (
    <div className="h-full min-h-0 overflow-hidden bg-slate-900/95 shadow-[-20px_0_50px_rgba(0,0,0,0.45)] relative z-30 grid grid-rows-[auto_minmax(0,1fr)]">
      <div className="px-4 lg:px-5 pt-4 pb-3 bg-slate-900/90 backdrop-blur-sm shadow-xl border-b border-slate-500/50 relative z-20">
        <div className="flex items-center gap-2.5 mb-3">
          <span
            className={cn(
              'px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-[0.18em] border shadow-[0_0_15px_rgba(0,0,0,0.2)]',
          ...
          <span className="text-[9px] font-mono text-slate-200/90 uppercase tracking-widest">
            ID: {selectedNode.id.toUpperCase()}
          </span>
        </div>

        <h2 className="text-xl lg:text-2xl font-serif font-black text-white leading-tight tracking-tight">
          {selectedNode.label}
        </h2>
      </div>
      ...
      >
        <div className="px-4 lg:px-6 py-1.5 border-b border-slate-600/70">
          <TabsList className="w-full grid grid-cols-3 bg-slate-800/95 border border-slate-500/80">
            <TabsTrigger value="visual" className="text-xs font-semibold">图解</TabsTrigger>
            <TabsTrigger value="analysis" className="text-xs font-semibold">解析</TabsTrigger>
            <TabsTrigger value="links" className="text-xs font-semibold">关联</TabsTrigger>
          </TabsList>
        </div>
        <div className="h-full min-h-0">
          ...
          >
            <motion.div
              key={selectedNode.id}
              className="px-4 lg:px-6 py-4 lg:py-5 space-y-7 pb-12"
            ...
            >
              ...
              >
                <h3 className="text-xs uppercase tracking-[0.18em] text-cyan-100 font-semibold">图解</h3>
                {visualComponent ? (
              ...
              >
                <h3 className="text-xs uppercase tracking-[0.18em] text-cyan-100 font-semibold">解析</h3>
                <div className="prose prose-invert prose-sm max-w-none prose-headings:font-sans prose-headings:font-bold prose-headings:tracking-tight prose-headings:text-slate-100 prose-h3:text-xl prose-h3:text-cyan-100 prose-h3:mt-8 prose-h3:mb-4 prose-h3:border-b prose-h3:border-slate-700 prose-h3:pb-2 prose-h4:text-xs prose-h4:uppercase prose-h4:tracking-widest prose-h4:text-slate-300 prose-h4:mt-8 prose-p:text-slate-200 prose-p:leading-7 prose-p:mb-4 prose-strong:text-white prose-strong:font-bold prose-li:text-slate-200 prose-li:marker:text-slate-500 prose-blockquote:border-l-cyan-400 prose-blockquote:bg-slate-800/60 prose-blockquote:py-3 prose-blockquote:px-5 prose-blockquote:rounded-r-lg prose-blockquote:not-italic prose-blockquote:text-slate-100 hr:border-slate-700 hr:my-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedNode.relatedConcepts.map((concept, index) => (
                        <span
                          ...
                          className="group cursor-help flex items-center text-xs font-medium bg-slate-900 border border-slate-600/80 text-slate-200 px-3 py-1.5 rounded-lg transition-all hover:border-cyan-400/60 hover:text-cyan-100 hover:bg-cyan-950/30"
                        >
                          <Lightbulb size={12} className="mr-2 text-slate-300 group-hover:text-yellow-300 transition-colors" />
                          {concept}
                        </span>
                      ))}
                    </div>
              ...
              <section
                ...
                className="space-y-4 scroll-mt-6"
              >
                <h3 className="text-xs uppercase tracking-[0.18em] text-cyan-100 font-semibold">关联</h3>
                <div className="text-xs text-slate-200 bg-slate-800/60 border border-slate-600/70 rounded-lg px-3 py-2">
